...
class ErrorsChart extends PureComponent {
    ...
    Promise.all([errors.fetchErrorPercentage(props.api, { ...props.primaryRange, interval: props.interval })]).then(data => {
      this.setState(prevState => {
        ...
        return {
          ...prevState,
          errors: [{
            data: data[0].map(day => { return { name: moment(day[0]).format(formats[props.interval]), y: util.roundTo(day[3], 4) }; }),
            type: 'spline',
  ...
  getMaxErrorPercentage (data) {
    return Math.max(d3.max(data[0], (x => { return util.roundTo(x[3], 4) }))) * 3;
  }
  render () {
    const chartConfig = {
      chart: {
    ...
    return <Card fixedHeight={true} title="Errors" width={ this.props.width || { md: 8, sm: 8, xs: 12 }} cardHeight={"480px"}>
