...
class DataQueryColumn extends Component {
  render () {
    const filters = this.props.column.filters.map((filter, index) => {
      return <DataFilter key={filter.key} colIndex={this.props.index} index={index} column={this.props.column} filter={filter} />
    });
    return (
    <div className="data-query column">
      <div className="col-md-6">
        <legend>Column Settings</legend>
        <FormSelect onChange={this.props.changeQueryFunction } label="Aggregate Function" defaultValue={this.props.column.queryFunction}>
        ...
        <DataFieldSelector label="Dimension" value={this.props.column.queryField} onChange={this.props.changeQueryField} />
      </div>
      <div className="col-md-6" style={{ borderLeft: '1px solid #c0c0c0' }}>
        ...
        <center><button className="btn btn-info" onClick={this.props.addFilter}>Add another Filter</button></center>
      </div>
    </div>);
  }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(DataQueryColumn);
