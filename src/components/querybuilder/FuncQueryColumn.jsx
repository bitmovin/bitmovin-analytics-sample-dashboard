...
const FuncQueryColumn = ({column, index, changeName, args, changeArgs, changeFormat}) => {
  ...
  return (
    <div>
      Arguments:
      <FormSelect
        label="Argument Column 1"
        defaultValue={column.args[0]}
        onChange={evt => {
          changeArgs(0, evt.target.value);
        }}>
        {argOptions}
      </FormSelect>
      <FormSelect
        label="Argument Column 2"
        defaultValue={column.args[1]}
        onChange={evt => {
          changeArgs(1, evt.target.value);
        }}>
        {argOptions}
      </FormSelect>
      <FormSelect
...
const mapStateToProps = (state, ownProps) => {
  return {
    ...
    args: state.query.columns.slice(0, ownProps.index).map((col, index) => {
      return {value: '$' + index, name: col.title};
    }),
  };
};
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    ...
    changeFormat: format => {
      dispatch(actions.changeFunctionColumnFormat(ownProps.index, format));
    },
  };
};
