...
class QueryColumn extends Component {
  ...
  render () {
    ...
    let titleOrEditor = <h4 className="card-title" onClick={() => { this.props.startTitleEdit(); }}>{this.props.column.title}</h4>;
    if (this.props.editingTitle === true) {
      titleOrEditor = <div><input ref="title" type="text" defaultValue={this.props.column.title} onBlur={(evt) => { this.props.changeTitle(evt.target.value); }} /></div>
    }
    return (
			<div className="card">
            ...
            <div className="text-right">
              <ToggleButton checked={this.props.renderInGraph} onChange={(visibility) => { this.props.changeRenderInGraph(visibility) }} label="Show as Graph line" />
              <button className="btn btn-danger btn-xs removeColumn" onClick={this.props.removeColumn}>
                <i className="fa fa-times-circle"></i> Remove Column
              </button>
            </div>
