    ...
    const rows = this.state.topContents.map((video, index) => {
      return (
        <tr key={index}>
          ...
          <td>{video.rebufferPercentage + ' %'}</td>
          <td>{video.startuptime + ' ms'}</td>
          <td>{video.errors}</td>
        </tr>
      );
    });
  ...
  render() {
    return (
      <Card title="Top Contents" width={this.props.width || {md: 12, sm: 12, xs: 12}} cardHeight={'480px'}>
        <LoadingIndicator loading={this.state.loading}>{this.renderTable()}</LoadingIndicator>
      </Card>
    );
  }
}
