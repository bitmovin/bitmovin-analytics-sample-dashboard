...
class Point extends Component {
  ...
  render () {
    return <div style={{position: 'absolute', width: '40px', height: '40px', left: '-20px', top: '-40px', textAlign: 'center'}}>
      <i className="fa fa-map-marker" style={{fontSize: '40px', color: 'black'}} aria-hidden="true"></i>
    </div>
  }
}
...
class PointLocationMap extends Component {
  ...
  render () {
    ...
    if (this.state.loaded === true) {
      point = <Point lat={this.state.lat} lng={this.state.lng} text={this.props.name}>{this.props.name}</Point>
    }
