    ...
    const rows = top.map((row, index) => {
      return (
        <tr key={index}>
          ...
          <td>{Math.round(row[3] * 10000) / 100 + '%'}</td>
        </tr>
      );
    });
    return (
    ...
    return (
      <Card width={this.props.width || {md: 8, sm: 8, xs: 12}} title="Rebuffer Percentage by Country" cardHeight="auto">
        <LoadingIndicator loading={this.state.loading}>
          <div className="col-md-6 col-sm-12 col-xs-12">{this.renderTable()}</div>
          <div className="col-md-6 col-sm-12 col-xs-12">{this.renderChart()}</div>
        </LoadingIndicator>
      </Card>
    );
  }
