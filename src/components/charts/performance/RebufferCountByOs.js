...
class RebufferCountGraphByOs extends Component {
  ...
  componentDidMount () {
    ...
    const filterUninterestingOss = (result) => {
      let systems = [];
      let max = 0;
      for (let os of Object.keys(result)) {
    ...
    rebuffer.rebufferPercentageGrouped(this.props.apiKey, ranges.thisWeek, 'OPERATINGSYSTEM').then(result => {
      result.sort((a,b) => { return a[0] - b[0] });
      result = _.groupBy(result, (row) => {
        return row[1];
      })
      result = filterUninterestingOss(result);
  ...
  render () {
    return <div>
      <Card cardHeight="auto" title="Rebuffer Percentage by Os" width={this.props.width || { sm: 6, md: 6, xs: 12}}>
