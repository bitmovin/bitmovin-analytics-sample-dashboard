...
class ImpressionDetail extends PureComponent {
  ...
  impressionList () {
    const list = this.props.impressions.map((row, index) => {
      return <tr key={index}>
          <td>{index}</td>
          <td>{moment(moment(row.time)).local().format('YYYY-MM-DD HH:mm:ss')}</td>
          <td>{row.state}</td>
          ...
          <td>{ row.played }</td><td>{row.seeked}</td>
  ...
  impressionBackgroundLoadBanner () {
    ...
    return <div className="alert alert-primary">
      <span><b>NOTE</b> This impression was loaded in a background tab so it's startuptime is not representative of the actual player startuptime. Delay is limited to time to first frame</span>
    </div>
  }
  ...
  renderUserLocationMap(startupImpression, height) {
    ...
    return (
      <PointLocationMap point={city} name={city} height={height} />
    );
  }
  ...
  render () {
    ...
    const getInfoRow = (name, attribute) => {
      if (this.props.isLoaded === true) {
        ...
        return <tr><td>{name}</td><td>Loading...</td></tr>
      }
    };
    ...
    if (this.props.isLoaded !== true) {
      ...
      return <div><div className="row">{this.impressionList()}</div></div>
    }
      ...
      <div className="row">
        <Card title="User Information" width={{ lg: 6, md: 12, sm: 12, xs: 12}} cardHeight={cardHeight + 'px'}>
          <table className="table">
            <tbody>
              <tr><td>Impression Type</td><td>{this.props.loadedInBackground ? 'Loaded in Background' : 'Loaded in Foreground' }</td></tr>
              <tr><td>Live</td><td>{this.props.isLive ? 'Yes' : 'No'}</td></tr>
              ...
              {getInfoRow('Browser', (imp) => { return `${imp.browser} ${imp.browser_version_major}.${imp.browser_version_minor}`; })}
              ...
              {getInfoRow('Screen Size', (imp) => { return `${imp.screen_width}x${imp.screen_height}` })}
              {getInfoRow('Language', 'language')}
              {getInfoRow('Visited Page', (imp) => { return <a href={urljoin('http://', imp.domain, imp.path)} target="_blank">{urljoin(imp.domain, imp.path)}</a>})}
...
export default connect(mapStateToProps, mapDispatchToProps)(ImpressionDetail)
