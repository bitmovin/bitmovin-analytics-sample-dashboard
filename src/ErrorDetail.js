...
class ErrorDetail extends Component {
    ...
    videosByErrorCode(props.api, props.interval, parseInt(props.errorCode, 10), {...props.primaryRange, limit, offset}).then((data) => {
      this.setState(prevState => {
        return {
          ...prevState,
          loaded: true,
          videos: data.map(p => { return { video: p[1], errors: p[2] }})
        }
      })
    });
  ...
  renderErrorsTable() {
    ...
    const videos = this.state.videos.map((video, index) => {
      return (<tr key={index}>
        <td><a className="error-detail-video-id" onClick={() => {this.setState({videoId: video.video}); this.loadErrorDetailsForVideoId(this.props, video.video)}}>{video.video}</a></td>
        <td>{video.errors}</td>
      </tr>)
    });

    return <div>
      <Card title={ "Error #" + this.props.errorCode + " - " + ErrorCodes[this.props.errorCode] } width={{xs: 12}} cardHeight={"180px"}>
        <p>
          This View gives you an overview where Error #{this.props.errorCode} - {ErrorCodes[this.props.errorCode]} occured and gives you a breakdown of what Technology caused the Error.
        </p>
        <p>Click a Video ID to see details for this Error in the clicked Video</p>
        <p>See documentation about the Error: <a target="_blank" href={"https://bitmovin.com/errors/?post_type=errors&s=" + this.props.errorCode}>Bitmovin Error Documentation</a></p>
      </Card>
  ...
  renderChart(title, series) {
    ...
    return <Card title={title} width={this.props.width || {md: 12, sm: 12, xd: 12}} cardHeight="500px">
  ...
  renderTable(title, data) {
    ...
    return <Card title={title} width={{xs: 6}} cardHeight={"480px"}>
      <table className="table table-hover">
        <thead>
        <tr>
          <th>{title}</th>
