...
function fetchMetric(api, metric, aQuery = {}) {
  ...
  return new Promise(resolve => {
    Promise.all([api.fetchAnalytics('COUNT', query), api.fetchAnalytics('COUNT', previousWeekQuery)]).then(results => {
...
export function fetchTotalTimePlayedThisWeek(api, impression, videoId) {
  return new Promise(resolve => {
    ...
    played.averagePlayedTimePerVideo(api, videoId, ranges.thisWeek).then(result => {
      resolve(
        formatResult(totalPlayedTime, result, value => {
...
export function fetchRebufferPercentage(api, impression, videoId) {
  return new Promise(resolve => {
    ...
    const filters = [api.filter('PLAYER_STARTUPTIME', 'EQ', 0), api.filter('VIDEO_STARTUPTIME', 'EQ', 0)];
    ...
    Promise.all(promises).then(result => {
      ...
      resolve(
        formatResult(rebufferPercentage, averageRebufferTimePerImpression, value => {
          if (value > 10000) {
...
export function fetchTimeToFirstFrame(api, impression, videoId) {
  const timeToFirstFrame = impression.find(x => {
    return x.video_startuptime > 0;
  }).video_startuptime;
  return new Promise(resolve => {
    ...
    api
      ...
      .then(result => {
        resolve(
          formatResult(timeToFirstFrame, result[0][0], value => {
            if (value > 10000) {
              return util.round10(value / 1000) + ' sec';
            }
            return Math.round(value) + ' ms';
...
export function fetchStartupTime(api, impression, videoId) {
  const startupTime = impression.find(x => {
    return x.startuptime > 0;
  }).startuptime;
  return new Promise(resolve => {
    const filters = [api.filter('STARTUPTIME', 'GT', 0), api.filter('PAGE_LOAD_TYPE', 'EQ', 1)];
    ...
    api
      ...
      .then(result => {
        resolve(
          formatResult(startupTime, result[0][0], value => {
            if (value > 10000) {
              return util.round10(value / 1000) + ' sec';
            }
            return Math.round(value) + ' ms';
...
export function fetchAverageBitrate(api, impressions, videoId) {
  const avgBitrate = mean(
    impressions
      .map(imp => {
        return imp.video_bitrate;
      })
      .filter(imp => {
        return imp > 0;
      })
  ...
  return new Promise(resolve => {
    ...
    api
      ...
      .then(result => {
        resolve(
          formatResult(avgBitrate, result[0][0], value => {
...
export function fetchRebufferPercentageThisWeek(api) {
  return new Promise(resolve => {
    Promise.all([
      rebuffer.rebufferPercentage(api, ranges.thisWeek),
      rebuffer.rebufferPercentage(api, ranges.lastWeek),
    ]).then(result => {
      resolve(
        formatResult(result[0], result[1], value => {
...
export function fetchImpressionsLastDaysPerDay(api, days, offset = 0) {
  ...
  return new Promise(resolve => {
    api.fetchAnalytics('COUNT', lastDaysQuery(days, offset)).then(results => {
      const data = results
        .sort((a, b) => {
          return a[0] - b[0];
        })
...
export function fetchBounceRateThisWeek(api) {
  return new Promise(resolve => {
    Promise.all([bounceRate(api, ranges.thisWeek), bounceRate(api, ranges.lastWeek)]).then(result => {
...
export function fetchAverageStartupDelayThisWeek(api) {
  return new Promise(resolve => {
    Promise.all([
      startupDelay.fetchPlayerStartupDelay(api, ranges.thisWeek),
...
export function fetchAverageVideoStartupDelayThisWeek(api) {
  return new Promise(resolve => {
    Promise.all([
      startupDelay.fetchVideoStartupDelay(api, ranges.thisWeek),
...
function fetchHeatmap(api, functionName, dimension, videoId, dataFormatter = noopFormatter) {
  ...
  return new Promise(resolve => {
    Promise.all([api.fetchAnalytics('MAX', query)]).then(results => {
