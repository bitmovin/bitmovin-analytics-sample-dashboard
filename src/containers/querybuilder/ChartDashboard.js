...
const mapStateToProps = (state, ownProps) => {
  ...
  const renderColumns = query.columns.map((col, index) => { return [col.renderInGraph, col.title, index]; }).filter(x => x[0] === true);
  return {
    data,
    query,
    editing,
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    refresh: () => { dispatch(loadDashboardData(ownProps.data.boardId, ownProps.data.query)); },
    remove: () => { dispatch(removeDashboard(ownProps.data.boardId)) },
    edit: () => {
